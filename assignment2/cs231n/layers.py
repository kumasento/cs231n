...
def affine_backward(dout, cache):
  ...
  db = np.sum(dout, axis=0).T # by chance, I don't know why it works
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx, dw, db
...
def relu_backward(dout, cache):
  ...
  #############################################################################
  dx = dout
  dx[x < 0] = 0 # found answer in another lecture
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  H_ = 1 + (H + 2 * conv_param['pad'] - HH) / conv_param['stride']
  W_ = 1 + (W + 2 * conv_param['pad'] - WW) / conv_param['stride']
  out = np.zeros((N, F, H_, W_))
  for i in range(N):
    xi = x[i]
    h_pad = w_pad = (conv_param['pad'], conv_param['pad'])
    xi_pad = np.pad(xi, ((0, 0), h_pad, w_pad), 'constant')
    # for each filter
    for j in range(F):
      wj = w[j]
      # then iterate all the point in result matrix
      for x_ in range(H_):
        for y_ in range(W_):
          x_h_pos = x_ * conv_param['stride']
          x_w_pos = y_ * conv_param['stride']
          out[i, j, x_, y_] = np.sum(xi_pad[:, x_h_pos:x_h_pos+HH, x_w_pos:x_w_pos+WW] * wj) + b[j]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
  ...
  for i in range(N):
    for c in range(C):
      ...
      for x_ in range(H_):
        for y_ in range(W_):
          h_pos, w_pos = x_ * stride, y_* stride
          x_ic_block = x_ic[h_pos:h_pos+pool_height, w_pos:w_pos+pool_width]
          x_ic_max_idx = np.argmax(x_ic_block)
          x_ic_max_h_idx = x_ic_max_idx / pool_width
          x_ic_max_w_idx = x_ic_max_idx % pool_width

          dx[i, c, h_pos+x_ic_max_h_idx, w_pos+x_ic_max_w_idx] += dout[i, c, x_, y_]
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
